...
def verificar_login(email, senha_digitada):
    ...
    try:
        ...
        if usuario:
            id_admin, nome, email_db, senha = usuario
...
def editar_morador(cpf, nome, email, id_residencia, sindico):
    ...
    try:
        ...
        WHERE cpf = %s"""
        atributos = (nome, email, id_residencia, sindico, cpf)
        cursor.execute(sql, atributos)
        conexao.commit()
        return True, "Dados do morador atualizados com sucesso!"
    except mysql.connector.Error as err:
        return False, f"Erro ao atualizar morador: {err}"
...
def obter_telefone_por_cpf(cpf):
    cursor = conexao.cursor()
    try:
        sql = "SELECT id_telefone_morador, numero FROM TELEFONE_MORADOR WHERE cpf_morador = %s LIMIT 1"
        cursor.execute(sql, (cpf,))
        return cursor.fetchone()
    except Exception as err:
        st.error(f"Erro ao buscar telefone: {err}")
        return False
    finally:
        cursor.close()
